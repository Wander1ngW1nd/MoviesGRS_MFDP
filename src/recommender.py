...
class GroupRecommender:
    ...
    def _preprocess_group_ratings(self, group_ratings: pd.DataFrame) -> pd.DataFrame:
        ratings_with_movie_id: pd.DataFrame = group_ratings.merge(self._movies_data, left_on="movie", right_on="title")
        ...
        ratings_with_movie_id["userId"] = ratings_with_movie_id.username.apply(lambda x: userid_mapping[x])
    ...
    def _get_unwatched_movies_ids(self, group_ratings: pd.DataFrame) -> np.array:
        ...
        watch_history: pd.DataFrame = group_ratings.groupby(by="userId").agg({"movieId": list}).reset_index()
        ...
        unwatched_movies_ids: np.array
        if self.group_size == 1:
            unwatched_movies_ids = watch_history.loc[0, "unwatched"]
        else:
            unwatched_movies_ids = np.array(
    ...
    def _get_users_embeddings(self, group_ratings: pd.DataFrame) -> pd.DataFrame:
        users_data = group_ratings.groupby("userId").agg({col: list for col in ["movieId", "rating"]}).reset_index()
        ...
        users_data["embedding"] = users_data.apply(
            lambda row: np.mean(
                self._movies_embeddings[self._movies_embeddings.movieId.isin(row["movieId"])].embedding * row["rating"],
                axis=0,
            ),
            axis=1,
        )
    ...
    def get_embedding_based_recommendation(
        ...
        unwatched_movies_embeddings: np.array = np.stack(
            unwatched_movies_data.merge(self._movies_embeddings, on="movieId").embedding.values
        )

        users_embeddings: pd.DataFrame = self._get_users_embeddings(group_ratings)
        avg_user_embedding: np.array = np.mean(users_embeddings.embedding, axis=0).reshape(1, -1)
    ...
    def get_svd_based_recommendation(self, group_ratings: pd.DataFrame, unwatched_movies_ids: np.array) -> np.array:
        ...
        self._svd.fit(surprise_train_dataset.build_full_trainset())

        movie_pseudorating: np.array = self._svd.bi[unwatched_movies_ids] + (
            self._svd.qi[unwatched_movies_ids] @ np.mean(self._svd.pu[self._starting_user_id :], axis=0)
        )
    ...
    def make_recommendation(self, group_ratings: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.group_size <= 3:
            ...
            top_movies = self.get_embedding_based_recommendation(group_ratings, unwatched_movies_data)
        else:
            top_movies = self.get_svd_based_recommendation(group_ratings, unwatched_movies_ids)

        recommendation: pd.DataFrame = self._movies_data[self._movies_data.movieId.isin(top_movies)][
            [
                "title",
                "year",
                "genres",
